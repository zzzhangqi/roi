...
func (o *SystemOptimizer) Run() error {
	...
	for i, host := range o.config.Hosts {
		o.logger.Infof("主机 %s: 开始系统优化...", host.IP)
		...
		if err := o.disableFirewalld(host); err != nil {
			o.logger.Warnf("主机 %s: 禁用防火墙服务失败: %v", host.IP, err)
		}

		if err := o.disableUFW(host); err != nil {
			o.logger.Warnf("主机 %s: 禁用UFW防火墙失败: %v", host.IP, err)
		}

		if err := o.disableSELinux(host); err != nil {
			o.logger.Warnf("主机 %s: 禁用SELinux失败: %v", host.IP, err)
		}

		if err := o.disableSwap(host); err != nil {
			o.logger.Warnf("主机 %s: 禁用交换分区失败: %v", host.IP, err)
		}

		if err := o.optimizeKernelParameters(host); err != nil {
			o.logger.Warnf("主机 %s: 优化内核参数失败: %v", host.IP, err)
		}

		if err := o.optimizeSystemLimits(host); err != nil {
			o.logger.Warnf("主机 %s: 优化系统限制失败: %v", host.IP, err)
		}

		o.logger.Infof("主机 %s: 系统优化完成", host.IP)
	}
...
func (o *SystemOptimizer) disableFirewalld(host config.Host) error {
	o.logger.Infof("主机 %s: 禁用firewalld防火墙...", host.IP)

	// 检查 firewalld 是否安装
	sshCmd := o.buildSSHCommand(host, "command -v firewall-cmd")
	if err := sshCmd.Run(); err != nil {
		o.logger.Infof("主机 %s: 系统未安装firewalld", host.IP)
		return nil
	}
	...
	if !isActive && !isEnabled {
		o.logger.Infof("主机 %s: firewalld已禁用，跳过操作", host.IP)
		return nil
	}

	if isActive {
		// 停止firewalld服务
		o.logger.Infof("主机 %s: 停止firewalld服务", host.IP)
		sshCmd = o.buildSSHCommand(host, "systemctl stop firewalld")
		if err := sshCmd.Run(); err != nil {
			return fmt.Errorf("停止firewalld服务失败: %w", err)
		}
		o.logger.Infof("主机 %s: 成功停止firewalld服务", host.IP)
	}

	if isEnabled {
		// 禁用firewalld服务
		o.logger.Infof("主机 %s: 禁用firewalld开机自启", host.IP)
		sshCmd = o.buildSSHCommand(host, "systemctl disable firewalld")
		if err := sshCmd.Run(); err != nil {
			return fmt.Errorf("禁用firewalld开机自启失败: %w", err)
		}
		o.logger.Infof("主机 %s: 成功禁用firewalld开机自启", host.IP)
	}
...
func (o *SystemOptimizer) disableUFW(host config.Host) error {
	o.logger.Infof("主机 %s: 禁用UFW防火墙...", host.IP)

	// 检查 UFW 是否安装
	sshCmd := o.buildSSHCommand(host, "command -v ufw")
	if err := sshCmd.Run(); err != nil {
		o.logger.Infof("主机 %s: 系统未安装UFW", host.IP)
		return nil
	}
	...
	if !firewallActive && !serviceActive && !serviceEnabled {
		o.logger.Infof("主机 %s: UFW已完全禁用，跳过操作", host.IP)
		return nil
	}

	// 禁用 UFW 防火墙
	if firewallActive {
		o.logger.Infof("主机 %s: 禁用UFW防火墙规则", host.IP)
		sshCmd = o.buildSSHCommand(host, "ufw --force disable")
		if err := sshCmd.Run(); err != nil {
			o.logger.Warnf("主机 %s: 禁用UFW防火墙规则失败: %v", host.IP, err)
		} else {
			o.logger.Infof("主机 %s: 成功禁用UFW防火墙规则", host.IP)
		}
	}

	// 停止 UFW 服务
	if serviceActive {
		o.logger.Infof("主机 %s: 停止UFW服务", host.IP)
		sshCmd = o.buildSSHCommand(host, "systemctl stop ufw")
		if err := sshCmd.Run(); err != nil {
			o.logger.Warnf("主机 %s: 停止UFW服务失败: %v", host.IP, err)
		} else {
			o.logger.Infof("主机 %s: 成功停止UFW服务", host.IP)
		}
	}

	// 禁用 UFW 服务开机自启
	if serviceEnabled {
		o.logger.Infof("主机 %s: 禁用UFW开机自启", host.IP)
		sshCmd = o.buildSSHCommand(host, "systemctl disable ufw")
		if err := sshCmd.Run(); err != nil {
			o.logger.Warnf("主机 %s: 禁用UFW开机自启失败: %v", host.IP, err)
		} else {
			o.logger.Infof("主机 %s: 成功禁用UFW开机自启", host.IP)
		}
	}
...
func (o *SystemOptimizer) disableSELinux(host config.Host) error {
	o.logger.Infof("主机 %s: 禁用SELinux...", host.IP)

	// 检查 SELinux 是否安装
	sshCmd := o.buildSSHCommand(host, "command -v getenforce")
	if err := sshCmd.Run(); err != nil {
		o.logger.Infof("主机 %s: 系统未安装SELinux", host.IP)
		return nil
	}
	...
	if currentStatus == "Disabled" && configStatus == "disabled" {
		o.logger.Infof("主机 %s: SELinux已完全禁用，跳过操作", host.IP)
		return nil
	}

	o.logger.Infof("主机 %s: 当前SELinux状态: %s，配置文件状态: %s", host.IP, currentStatus, configStatus)

	// 临时禁用 SELinux（如果当前是启用状态）
	if currentStatus == "Enforcing" || currentStatus == "Permissive" {
		o.logger.Infof("主机 %s: 临时禁用SELinux", host.IP)
		sshCmd = o.buildSSHCommand(host, "setenforce 0")
		if err := sshCmd.Run(); err != nil {
			o.logger.Warnf("主机 %s: 临时禁用SELinux失败: %v", host.IP, err)
		} else {
			o.logger.Infof("主机 %s: 成功临时禁用SELinux", host.IP)
		}
	}

	// 永久禁用 SELinux（修改配置文件）
	if configStatus != "disabled" {
		o.logger.Infof("主机 %s: 永久禁用SELinux（修改配置文件）", host.IP)
		sshCmd = o.buildSSHCommand(host, "sed -i 's/^SELINUX=.*/SELINUX=disabled/' /etc/selinux/config")
		if err := sshCmd.Run(); err != nil {
			return fmt.Errorf("永久禁用SELinux失败: %w", err)
		}
		o.logger.Infof("主机 %s: 成功永久禁用SELinux（需要重启生效）", host.IP)
	}
...
func (o *SystemOptimizer) disableSwap(host config.Host) error {
	o.logger.Infof("主机 %s: 禁用交换分区...", host.IP)
	...
	if !swapActive && !swapInFstab {
		o.logger.Infof("主机 %s: 交换分区已完全禁用，跳过操作", host.IP)
		return nil
	}

	// 禁用当前激活的交换分区
	if swapActive {
		o.logger.Infof("主机 %s: 关闭当前激活的交换分区", host.IP)
		sshCmd = o.buildSSHCommand(host, "swapoff -a")
		if err := sshCmd.Run(); err != nil {
			o.logger.Warnf("主机 %s: 关闭交换分区失败: %v", host.IP, err)
		} else {
			o.logger.Infof("主机 %s: 成功关闭激活的交换分区", host.IP)
		}
	}

	// 注释掉 /etc/fstab 中的交换分区条目
	if swapInFstab {
		o.logger.Infof("主机 %s: 注释/etc/fstab中的交换分区条目", host.IP)
		sshCmd = o.buildSSHCommand(host, "sed -i '/swap/s/^/#/' /etc/fstab")
		if err := sshCmd.Run(); err != nil {
			return fmt.Errorf("注释/etc/fstab中的交换分区条目失败: %w", err)
		}
		o.logger.Infof("主机 %s: 成功注释/etc/fstab中的交换分区条目", host.IP)
	}
...
func (o *SystemOptimizer) optimizeKernelParameters(host config.Host) error {
	o.logger.Infof("主机 %s: 优化内核参数...", host.IP)
	...
	if err := sshCmd.Run(); err == nil {
		o.logger.Infof("主机 %s: 内核参数已优化，跳过操作", host.IP)
		return nil
	}
...
net.ipv4.tcp_synack_retries=2`
	...
	// 写入 sysctl 配置
	o.logger.Infof("主机 %s: 写入内核参数配置文件", host.IP)
...
func (o *SystemOptimizer) optimizeSystemLimits(host config.Host) error {
	...
	if err := sshCmd.Run(); err == nil {
		o.logger.Infof("主机 %s: 系统限制已优化，跳过操作", host.IP)
		return nil
	}
...
func (o *SystemOptimizer) buildSSHCommand(host config.Host, command string) *exec.Cmd {
	...
	if host.Password != "" {
		...
		if _, err := exec.LookPath("sshpass"); err != nil {
			o.logger.Warnf("未找到sshpass工具，请安装sshpass或为主机 %s 使用SSH密钥认证", host.IP)
